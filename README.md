...
•	You likely will need to use at least MATLAB 2017b, but I coded most of it in 2019
...
• When launching a GUI, we will check if there are any matching data objects in the base workspace and ask user if he wants to import these, but you can also simply load the data from raw within the GUI or load a GUI state that you saved to disk
...
#### GUI menu bar:
* File:
     * Set default parameters for all aspects of GUI or restore the built-in defaults. Note that the directory in the GUI defaults is the location on disk where data will be saved to
     * Delete currently selected dataset(s), trial(s) or cell(s)
     * Save GUI state; either the currently selected dataset(s) or the full GUI content 

* Settings:
     * Load Datasets from raw or a GUI state
     * Pause updates for any plot(s) on the _Overview_ tab in case updating is slow or plot(s) are irrelevant for user 

* Figures:
     * Save figures generated by GUI as PDFs to disk (either all or custom selection)
