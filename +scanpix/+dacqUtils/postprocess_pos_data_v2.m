function [xy, dir, speed, stats] = postprocess_pos_data_v2(posdata, max_speed, box_car, setfile_header, headPos )
...
%
stats(3,1) = length(swap_list) / size(posdata.led_pos,1);
...
for led = 1:n_leds
    ...
    if( n_jumpy > n_pos/3 )
        warning('scaNpix::dacqUtils::postprocess_pos_data_v2: %d/%d positions rejected for led %d\n', n_jumpy, n_pos, led);
    end
end
%
stats(1,:) = sum(~isnan(squeeze(posdata.led_pos(:,:,1))),1) / size(posdata.led_pos,1);
...
if length(ok_pos) < 2
    warning('scaNpix::dacqUtils::postprocess_pos_data_v2::led_speed_filter: < 2 tracked points for led %d\n', led);
end
