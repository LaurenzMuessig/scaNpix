...
%         csMissed = cumsum([0;nMissedPulses]);
%         x = 1:length(syncTTLs);
%         for i = 1:length(missedSyncs)
%             missedPulses    = [missedPulses,(missedSyncs(i)+1:missedSyncs(i)+nMissedPulses(i)) + csMissed(i)];
%             x(missedSyncs(i)+1:end) = x(missedSyncs(i)+1:end) + nMissedPulses(i); % bump sample points for interpolation
%         end
...
%         syncTTLs               = temp;
%         warning('scaNpix::ephys::loadSpikes: %i missing sync pulses across %i chunks in neuropixel datastream. Interpolated missing samples, but better go and check that',length(missedPulses),i);
%     end
% end
